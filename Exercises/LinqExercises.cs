...
public sealed class LinqExercises
{
    ...
    {
        return [UniversityData.Lecturers.Count(e => 
            !string.IsNullOrEmpty(e.Department)) == UniversityData.Lecturers.Count ? "Yes" : "No"];
    }
    ...
    {
        return UniversityData.Courses.OrderBy(c => c.Title).Skip(2).Take(2).Select(e
            => $"{e.Title} {e.Category}").ToList();
    }
    ...
    {
        return UniversityData.Students.Join(UniversityData.Enrollments, st => st.Id, e => e.StudentId, 
    ...
    {
        return UniversityData.Enrollments.Join(UniversityData.Students, e => e.StudentId, s => s.Id, (e, s) =>
            new { e, s }).Join(UniversityData.Courses, x => x.e.CourseId, c => c.Id, (x, c) =>
            $"{x.s.FirstName} {x.s.LastName} {c.Title}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.GroupBy(e => e.CourseId).Join(UniversityData.Courses, g => 
            g.Key, c => c.Id, (g, c) => $"{c.Title} {g.Count()}").ToList();
    }
    ...
    {
        return UniversityData.Enrollments.Where(e => e.FinalGrade != null).GroupBy(e => 
            e.CourseId).Join(UniversityData.Courses, g =>
            g.Key, c => c.Id, (g, c) => $"{c.Title} {g.Average(e => e.FinalGrade)}").ToList();
    }
